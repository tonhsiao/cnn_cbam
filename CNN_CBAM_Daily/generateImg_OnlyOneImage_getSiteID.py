...
ttdata = rawCott[rawCott.itt_id=='M2210030910']
ttdata.to_csv('D:\\Nicole\\python\\cottCNN\\M2210030910.csv')


ttdata = ttdata[['itt_id', 'site_id', 'n_type', 'start_time','duration','pos_first_rsrp', 'c_prbutil', 'c_rssi','end_cqi','call_type','dl_volume','dl_tput','pos_last_rsrq']]  
...
df_raw = df_raw0[['itt_id', 'site_id', 'n_type', 'start_time','EVENT_START_DATE','EVENT_DATE','duration','pos_first_rsrp', 'c_prbutil', 'c_rssi','end_cqi','call_type','dl_volume','dl_tput','pos_last_rsrq']]    
...
for i in range(len(itt_id)):
    ...
    site1 = get_site_id(df, 9, 12)
    site2 = get_site_id(df, 13, 17)
    site3 = get_site_id(df, 19, 24)
